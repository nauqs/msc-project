...
IFS=, read -r env_id time_cost action_cost fully_obs wandb seed total_timesteps num_envs num_steps ent_coef <<< "$LINE"
...
free -g
/cluster/project2/tithonus/miniconda3/bin/conda run -n minigrid python -u train.py --env-id $env_id --time-cost $time_cost --action-cost $action_cost --fully-obs $fully_obs --wandb $wandb --seed $seed --total-timesteps $total_timesteps --num-envs $num_envs --num-steps $num_steps --ent-coef $ent_coef --exp-name $exp_name $cuda_option
